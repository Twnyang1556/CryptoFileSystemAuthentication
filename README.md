...
### Why it's secure:
 Due to the system of using two different types of hashes both sides validate the equality of the files using a different hash so nothing would be revealed.  We use two algorithms, the server and client, in order to have each algorithm check a hash and confirm that their file hashes match without having the other sides file content revealed.
